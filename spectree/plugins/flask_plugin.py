...
class FlaskPlugin(WerkzeugPlugin):
    ...
    def get_request_data(self, request, endpoint: EndpointSpec) -> RequestData:
        ...
        use_json = endpoint.json and has_data and request.mimetype not in self.FORM_MIMETYPE
    ...
    def validate_response(self, resp, resp_model: Response | None, skip_validation: bool, force_resp_serialize: bool):
    ...
    def validate(self, func: Callable, endpoint: EndpointSpec, *args: Any, **kwargs: Any):
        if request.url_rule is not None and getattr(request.url_rule, "websocket", False):
        ...
        try:
            ...
            response = make_response(self.model_adapter.validation_errors(err), endpoint.validation_error_status)
        ...
        response, resp_validation_error = self.validate_response(result, endpoint.response, endpoint.skip_validation, endpoint.force_resp_serialize)
